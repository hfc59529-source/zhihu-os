...
| `docs/知乎内容质量参数库_V2.md` | 02 知识模块 | 是，质量参数权威（§0 QT-QI 另见下条 High-Intent Reader Rule 说明） | 总AI执行中心、GPT审核清单、DECISION（§0 QT-QI，2026-08-25 起含 QT-00/QI-01～QI-08、QI-04.1 与 QT-QI.1） | 生产协议只触发参数，不复制维护参数 |
| `docs/知乎内容质量参数库_V2.md`§0 QT-QI（High-Intent Reader Rule，2026-08-25） | 01 生产模块 | 是，DECISION 强制输入（Compiler V1 第4节）：Production Card 退役、职责迁入 Compiler V1 后，QT-QI 问题理解识别域由 2026-08-11 的六项 Migration Fix 扩展为完整问题理解链；DECISION 必须完成 QT-00/QI-01～QI-08、QI-04.1 与 QT-QI.1，识别提问者真实问题、观看者映射问题和 Personal Stakes，再锁定 Reality / Main Gap / Transformation / Core Judgment。该文件其余 PD/RR/RE/BT/CR 等正文质量参数消费者不变（仍是 COMPILE/WRITE/AUDIT），不因本次修复扩大范围 | DECISION（Reality/Main Gap/Transformation/Core Judgment 锁定前必须先完成） | 不得把 QT-QI 之外的参数域一并授权给 DECISION；不得把高意图阅读原则另建为平行模块；不得反向声称本条目扩大了`docs/知乎内容质量参数库_V2.md`对 DECISION 的整体授权 |
