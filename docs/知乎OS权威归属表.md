...
| Explanation Target（一致解释目标）字段定义 | 01 生产模块 | 是，跨节点字段，各字段权威分属不同节点，不归单一文件：`读者真实困惑`（INPUT，来源 `docs/Codex选题采集协议.md`§1.1）、`唯一核心判断`/Core Judgment（DECISION，来源 `docs/内容架构总则.md`）、`Reasoning Path`（COMPILE，来源 `docs/知乎OS Compiler V1.md`第5节）；`templates/Claude正文生产Prompt.md` 只在 WRITE 步骤消费这些字段，不持有其定义权威 | Claude正文生产Prompt、GPT审核清单 | 不得新增独立 Skill、Engine、流程节点、知识库或平行字段体系；不得把 INPUT/DECISION/COMPILE 字段的定义权威归给 WRITE 侧的 Claude正文生产Prompt |
