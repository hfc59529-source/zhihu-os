...
Decision Right:
  ...
  Core Judgment Discovery（2026-08-17 修复，2026-08-17 收窄为作者视角）：
    锁定 Core Judgment 前，不得直接把 Main Gap 平移成结论，也不得做中性的
    多角度分析（列出社会/管理/法律/心理等多个视角再挑一个"最全面"的）。
    按以下方向检查题面，找真实存在（而非虚构）的裂缝：
      - 表面主张和真实动机之间有没有裂缝？嘴上说的和实际会做的是不是两回事？
      - 谁获得好处、谁承担代价？说话的人是不是那个承担代价的人？
      - 这个观点/立场，轮到本人需要付出代价、承担风险、放弃利益时，
        是否仍然成立？
      - 体面的解释下面，是否存在更简单的人性和利益逻辑？
    只有确实找到真实裂缝/利益错位时，才允许用它形成或调整 Core Judgment；
    找不到时必须原样退出，不得为制造张力而虚构处境、动机或冲突，也不得
    退回去做中性多角度罗列。
