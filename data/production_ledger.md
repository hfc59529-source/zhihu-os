...
| ZH-20260804-003 | 为什么有任务的时候，领导从来不分配给他提拔的人，却分给业务能力过硬的普通员工？ | Answer_Benchmark_Top3（Topic Package 已采集，Claude 未重新打开知乎，见 TOPIC-20260804-003.md） | 人工评分 8.8/10（3 处 Approve，见 Decision_Log.md） | 1 | PATCH_VALIDATION_REQUIRED（Patch-v1 已生成为 Article-Patched-v1.md，解决唯一核心判断发散、清单腔、结尾抽象三处；待人工 Final Validation） | NOT_ENTERED | 待 Final Validation |
...
| ZH-20260802-001 | 提拔你当了部门负责人，第一件事该干啥？ | 未记录（见 Top3_Context.md / Benchmark_Brief.md） | 未走本流程 | 0 | DRAFT_READY（仅有 Card-v1 / Draft-v1，无 QA / 审计记录；未进入 READY_FOR_AUDIT） | NOT_ENTERED | 已实际发布（见下方说明），但本地生产链未走完 |
| ZH-20260808-001 | 公司实行「上四休三」，但要降薪1000元，员工不接受只能辞职，这样合理吗？换做是你会如何选择？ | 用户提供截图 | GPT QA PASS（Draft-v5，见 Production_Decision.md） | 4（Draft-v1→v5） | LEGACY_RELEASE_COMPLETED（Release-v1 已生成并已发布；Production_Decision.md 未记录任何 User Review / USER_APPROVED 证据，直接从 GPT QA PASS 进入 Release） | PUBLISHED（`data/production_article_map.csv`：answer_2069522411372933416，COMPLETE） | 待观察 |
| ZH-20260808-002 | 作为公司中层干部的你，发现老板非常喜欢绕过你，直接给基层的员工下达指令，遇到这样的情况你如何处理？ | 用户提供 / 知乎推荐 | QA-v1 Issue → QA-v2/v3 PASS | 2 | LEGACY_RELEASE_COMPLETED（Release-v1 已生成并已发布；Production_Decision.md 记录了两轮"User Review"文字过程，但未落成正式 `USER_APPROVED` 状态字段） | PUBLISHED（`data/production_article_map.csv`：answer_2069533843439235799，COMPLETE） | 待观察 |
| ZH-20260808-003 | 为什么很多中层永远无法晋升高层，从中层管理到高层管理，挑战在哪里？ | 知乎搜索/截图候选 | QA-v1 PASS | 0 | LEGACY_RELEASE_COMPLETED（Release-v1 已生成并已发布；Production_Decision.md 记录 Status 仍为 `RELEASED_AWAITING_PUBLISH`，与 `production_article_map.csv` 的 COMPLETE 状态不一致，本行按发布事实记录，Production_Decision.md 字段本身待更正） | PUBLISHED（`data/production_article_map.csv`：answer_2069537531780129888，COMPLETE） | 待观察 |
| ZH-20260809-001 | 为什么工人们宁愿联合起来就为了多加班多干活，也不愿意联合起来要求涨工资呢？ | 未记录 | 未记录（无 Production_Decision.md，仅有多版本 Draft） | 2+（Draft-v1/v2-progression-experiment/v3-writer-authority） | LEGACY_RELEASE_COMPLETED（Release-v1 已生成并已发布；无 Production_Decision.md，无法核实审计/用户验收过程） | PUBLISHED（`data/production_article_map.csv`：answer_2069838524929594559，COMPLETE） | 待观察 |
| ZH-20260809-002 | （见 STATUS.md，未记录标题） | 未记录 | 未走本流程 | 0 | INELIGIBLE — CONTAMINATED_SAMPLE（`productions/ZH-20260809-002/STATUS.md`：生成于 EXP003 实验性 Session 之后，无法排除 Context Contamination；系统自行判定 Production Eligibility: NO） | NOT_ENTERED（STATUS.md 明确"不进入正式发布队列"，不作为生产样本或效果证据使用） | 污染样本，不计入统计 |
| ZH-20260810-001 | （管理层决策权/责任对称问题，见 Execution_IR-v2.md） | Answer_Benchmark_Top3 | 未完成（Realization_Audit.md 发现 Activation→Realization 断点，触发 Proposal-A/B 治理流程） | 3（Draft-v1→v4，Execution_IR-v1→v2） | READY_FOR_AUDIT（v4，重新确认；见 Production_Decision.md「系统级 Governance Decision」，本批次是 Proposal-A/B 两份治理文件的直接触发样本） | NOT_ENTERED | 治理事件样本，待重新审计 |
| ZH-20260810-002 | 为什么很多企业推行"扁平化管理"后，效率反而下降了？ | 未记录 | AUDIT v1 Issue（1）→ REVIEW #1 Patch → REVIEW #2 USER_REJECTED（终审） | 1 | REJECTED（Draft-v3 经用户终审判定为"抽象机制逐项施工"结构性问题，同源于材料不足；生产终止，不进入 Release；记为 Failure Pattern FP-20260810-001，occurrence_count: 1，未达 3 次复现门槛，不修改 Runtime Rules） | NOT_ENTERED | 已完整走完 Compiler V1 全链路（DECISION→COMPILE→WRITE→AUDIT→REVIEW→Patch→REVIEW）首个样本 |
| ZH-20260811-001 | 为什么职场上的形式主义越来越严重了？ | Codex 创作中心推荐问题 / Answer_Benchmark_Top3 | AUDIT-v10 PASS → 发布前最终检查 USER_REJECTED / PUBLISH_ABORTED | 6+（Draft-v1→v7，Execution_IR-v1→v4，AUDIT-v1→v10） | PUBLISH_ABORTED（机器 Gate PASS 后，人工发布前最终检查判定内容价值不足；归属 Unknown，记为 FP-20260811-002，未满 3 次不修改 Runtime Rules） | NOT_ENTERED（Release-v1 仅归档，不得发布） | 未确认质量失败观察，等待复现 |
| ZH-20260811-002 | 为什么职场上的形式主义越来越严重了？ | TOPIC-20260811-001 / Gate A-B 最小实验 | AUDIT-v1 PASS → REVIEW feedback → Draft-v2 → AUDIT-v2 PASS | 1 | READY_FOR_USER_REVIEW（Draft-v2 / AUDIT-v2 PASS，等待用户明确 USER_APPROVED 或 USER_REJECTED） | NOT_ENTERED | Gate A/B 最小生产实验；不写成生产规则 |
| ZH-20260905-001 | 如何判断一个人是否有领导能力？ | Challenge Case：旧同题 answer_2072695242105660190（375 阅读 / 2 赞 / 0 评论 / 0 收藏） | 暂不审计正文 | 0 | CHALLENGE_CASE_LOCKED（先锁定旧框架失败归因与新框架变量差异；不急于写正文；见 Challenge_Case-v1.md / Semantic_Freeze-v1.md） | NOT_ENTERED | 框架干预实验：Trait Definition → Behavioral Diagnosis |

**关于上表 ZH-20260808-001/002/003、ZH-20260809-001 的 LEGACY_RELEASE_COMPLETED 标注说明**：这 4 篇发生在 `ZH-20260801-011` 起"必须严格执行 USER_APPROVED 门禁"的规则生效之后，不属于本文件第 5 条定义的"用户验收节点建立前"的历史批次；此处沿用 `LEGACY_RELEASE_COMPLETED` 仅为记录既成事实（Release-v1 已生成、已人工发布到知乎、已产生待回收收益），不代表这 4 篇符合当前 Release Gate 规则。这是一处治理缺口（发布绕过了 `Publish_Queue.md` 与 `USER_APPROVED` 门禁），不是可复制的正常路径，后续生产不应参照此 4 篇跳过用户验收直接发布。
