...
1. 当前选题可完整识别。
2. 已保存真实知乎问题链接。
3. 已执行历史资产检索；命中不足时必须标注“历史证据不足”，但不得单独阻止 Production Card。
4. `runtime/知乎结构库快照.md` 中存在可调用 ACTIVE 老师结构。
5. 已选择唯一 ACTIVE 结构。
6. 已读取该结构完整推进字段。
7. 已完成本题结构实例化。
8. 已生成本题因果追问链和推导终点。
9. 已为每段写出具体内容、现实场景或例子、推进关系。
10. 已生成或刷新 `runtime/production_variable_snapshot.md`；变量只作为内容材料，不作为正文骨架。
11. 已校验变量快照满足六层筛选顺序和标准字段。
12. 后台质量参数检查已完成。

题目链接规则：

- Production Card 的 `问题链接` 必须是真实知乎问题链接。
- 用户只提供题目文字或截图但无法识别链接时，必须先返回 `READY_FOR_LINK_INPUT`，不得生成正式 Production Card。
- 占位链接只允许用于本地校验测试，产物必须标记 `TEST_ONLY`，不得进入正文生产、审核或发布。
...
- 不新增 Explanation Target（一致解释目标）字段；一致解释目标由现有字段承载。
- `读者真实困惑` 承载题主真正困惑，必须从题主显性问题翻译而来。
- `因果追问链` 承载解释覆盖的相关变量及其因果顺序，必须始终围绕同一个读者真实困惑展开，不得写成变量清单、观点清单或并列提纲。
- `因果追问终点` 承载解释追到哪一层，必须落到能回答原问题的判断层。
- `唯一核心判断` 承载读者最终形成的判断，必须直接回答同一个读者真实困惑。
- `分段施工说明 / 推进关系` 承载每段如何继续解释同一个读者真实困惑，必须说明上一层解释如何推进到下一层解释。

生成纪律：

- Production Card 负责定义 Explanation Target（一致解释目标）。
- Claude 只负责执行 Card，不得自行重建解释对象。
- 每一层解释都必须继续回答同一个读者真实困惑；不得从该困惑滑到泛泛的组织、成长、权力、沟通或情绪话题。
- 如果无法让所有段落共同回答同一个读者真实困惑，必须停止并返回 `FAIL-CARD-EXPLANATION-TARGET-DRIFT`。
...
- FAIL-CARD-EXPLANATION-TARGET-DRIFT：因果追问链或分段施工说明切换了解释对象，没有持续回答同一个读者真实困惑。
